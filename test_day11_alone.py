...
if exercise_path and answer_path:
    ...
    while i < len(exercise_content):
        ...
        if current_section == 'choice':
            ...
            if options:
                questions.append({'type': 'choice', 'day': day_num, 'number': len(questions)+1, 'question': q_text, 'options': options, 'answer': None})
            i = j
            ...
            questions.append({'type': 'code', 'day': day_num, 'number': len(questions)+1, 'question': q_text, 'options': [], 'answer': None, 'code_hint': '\n'.join(code_lines)})
            i = j
    ...
    print(f'Day 11: {len(questions)}题')