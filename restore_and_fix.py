...
backup_data = json.load(open('exercises.json.bak', 'r', encoding='utf-8')) if __import__('os').path.exists('exercises.json.bak') else []

if backup_data:
    data = backup_data
else:
    import shutil
    shutil.copy('exercises.json', 'exercises.json.bak')
...
for item in data:
    ...
    if q.startswith('第') and '题' in q and (':' in q or '：' in q):
        if current_item:
            fixed_data.append(current_item)
