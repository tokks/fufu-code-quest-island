...
def fix_day(day_num):
    day_items = sorted([item for item in data if item.get('day') == day_num], key=lambda x: x['number'])
    if not day_items:
        return []
    ...
    for item in day_items:
        ...
        if q.startswith('第') and '题' in q and ('：' in q or ':' in q):
            if current_item:
                fixed_items.append(current_item)
            ...
            if item.get('code_hint'):
                current_item['code_hint'] = item['code_hint']
        
        elif q.startswith('题目要求') or q.startswith('具体要求'):
            if current_item:
                current_item['question'] += '\n' + q
                if item.get('code_hint'):
                    current_item['code_hint'] += '\n' + item['code_hint']
        ...
        elif q.startswith('2.使用') or q.startswith('3.') or q.startswith('4.'):
            if current_item:
                if item.get('code_hint'):
                    current_item['code_hint'] += '\n' + item['code_hint']
...
day_fixes = {}
for day in range(4, 22):
    fixed = fix_day(day)
    if fixed:
        day_fixes[day] = fixed
        print(f"Day {day}: {len(fixed)} 道题目")

data = [item for item in data if item.get('day') not in day_fixes]
...
answer_templates = {
    ...
    11: {
        ...
        2: "def add(*args):\n    total = 0\n    for num in args:\n        total += num\n    return total\nprint(add(1, 2, 3))\nprint(add(1, 2, 3, 4, 5))",
        3: "def print_info(**kwargs):\n    for key, value in kwargs.items():\n        print(key + ': ' + str(value))\nprint_info(name='张三', age=18, score=95)",
    ...
    13: {
        ...
        5: "class Car:\n    def __init__(self, brand, model):\n        self.brand = brand\n        self.model = model\n    def drive(self):\n        print(self.brand, self.model, '正在行驶')\ncar = Car('宝马', 'X5')\ncar.drive()"
    },
    14: {
        1: "class Animal:\n    def speak(self):\n        pass\nclass Dog(Animal):\n    def speak(self):\n        print('汪汪汪')\nclass Cat(Animal):\n    def speak(self):\n        pass\nd = Dog()\nd.speak()",
    ...
    17: {
        ...
        5: "__all__ = ['public_function', 'PublicClass']\ndef public_function():\n    pass\ndef _private_function():\n    pass\nclass PublicClass:\n    pass\nclass _PrivateClass:\n    pass"
    },
    18: {
...
for day, items in day_fixes.items():
    if day in answer_templates:
        for item in items:
            num = item['number']
            if num in answer_templates[day]:
                item['answer'] = answer_templates[day][num]
                item['type'] = 'code'
