...
def find_files(day_num):
    ...
    for filename in os.listdir(DOWNLOADS_DIR):
        if filename.endswith(".docx"):
            has_day = False
            if f"day_{day_str2}" in filename.lower():
                has_day = True
            elif f"day_{day_str}" in filename.lower():
                has_day = True
            elif f"day{day_str2}" in filename.lower():
                has_day = True
            elif f"day{day_str}" in filename.lower():
                has_day = True
...
def parse_day(day_num):
    ...
    print(f'Day {day_num}: 习题文件={os.path.basename(exercise_path)}')
    
    exercise_content = parse_docx(exercise_path)
    
    if answer_path:
        answer_content = parse_docx(answer_path)
        print(f'          答案文件={os.path.basename(answer_path)}')
    else:
        answer_content = []
        print(f'          未找到答案文件')
    ...
    while i < len(exercise_content):
        ...
        if "选择题" in line:
            ...
            i += 1
            continue
        elif f"day{day_num}" in line.lower() or f"day_{str(day_num).zfill(2)}" in line.lower():
            i += 1
            continue
